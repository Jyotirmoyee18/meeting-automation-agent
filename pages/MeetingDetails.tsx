...
function ActionItemRow({
  ...
  return (
    <div
      className={`flex items-start gap-3 p-3 rounded-xl transition-all ${item.completed ? "opacity-60" : ""}`}
    >
      <button
        onClick={onToggle}
        ...
        {filtered.map((seg) => (
          <div key={seg.id} className="p-4 hover:bg-slate-50">
            <div className="flex items-center gap-2 mb-1.5">
              <span
                className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full border ${speakerColor(seg.speaker)}`}
              >
                {seg.speaker !== null
            ...
            <p
              className={`text-sm text-slate-700 leading-relaxed ${
                query && seg.text.toLowerCase().includes(query.toLowerCase())
                  ? "bg-yellow-50 px-2 py-1 rounded"
                  : ""
              }`}
            >
              {seg.text}
...
const MeetingDetails: React.FC = () => {
  ...
  const retryAnalysis = async () => {
    ...
    setRetrying(true);
    try {
      await fetch(`${API_URL}/api/meetings/${id}/retry-analysis`, {
        method: "POST",
      });
      // Poll for completion
  ...
  const copyTranscript = () => {
    const text = segments
      .map((s) => {
        const label =
          s.speaker !== null ? `Speaker ${s.speaker + 1}` : "Speaker";
      ...
      <div className="max-w-5xl mx-auto px-8 py-8 space-y-6">
        ...
        {(!analysis || analysis.status === "failed") && (
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 flex items-start justify-between gap-4">
            <i className="fas fa-robot text-amber-500 text-xl mt-0.5 flex-shrink-0" />
            <div className="flex-1 min-w-0">
              ...
              {analysis?.error_message && (
                <p className="text-amber-600 text-sm mb-3 break-words">
                  {analysis.error_message}
                </p>
              )}
              <button
